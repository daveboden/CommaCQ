...
/**
 * Unmarshals CSV to beans that have Jaxb annotations and are expecting
 * a single XML element with attributes.
 * 
 * Not threadsafe (on purpose). Meant to be used by a single thread responsible for
 * performing the initial load of bean cache data and performing the updates.
 */
public class JaxbAttributeWriterStrategy<BeanType> implements CsvToBeanStrategy<BeanType> {
	...
	public JaxbAttributeWriterStrategy(Class<BeanType> beanType) {
		...
		try {
		    ...
		    while((rowValues = csvParser.getLine()) != null) {
	            ...
	            try {
		            ...
		            throw new RuntimeException("Error while unmarshalling: " + beanType +
		            		                   " - " + Arrays.toString(rowValues), ex);
	            }
	...
	/**
	 * Takes key/value pairs and pretends to create an XML tag like:
	 * <BeanClassname id="a" name="b" .../>
	 * The Bean must have Jaxb annotations. The XMLEventReader pumps
	 * events into the Jaxb unmarshaller and Jaxb produces an instance
	 * of the bean.
	 * 
	 * This class is designed to be reusable, with new data plugged
	 * in for each instance of the bean we want to create. This is just
	 * to avoid creating an extra object each time we want to create
	 * a bean. 
	 */
	static class AttributeEventReader implements XMLEventReader {
		...
		/**
		 * A reusable working list which gets cleared down between creating each tag.
		 * Saves allocating a new array list for each bean.
		 */
